...
describe("DateParser Suite", function() {
  ...
  it("should take a date and return datetime", function() {
    var aprilFirst = moment.utc('2013-04-01T12:00:00'); 
    expect(dateParser.getDateTime(aprilFirst)).to.equal("2013-04-01T12:00:00Z");
  });

  it("should parse D-{MONTH abbreviation}-YY to YYYY-MM-DD", function() {
    expect(dateParser.parseAsMySqlDate("22-OCT-13")).to.equal("2013-10-22");
  });
  ...
  it("should parse M/D/YY to YYYY-MM-DD", function() {
    expect(dateParser.parseAsMySqlDate("3/10/11")).to.equal("2011-03-10");
  });

  it("should handle leap year dates correctly", function() {
    //leap year
    expect(dateParser.parseAsMySqlDate("29-FEB-08")).to.equal("2008-02-29");
  });

  it("should return correct ISO dates if they are already in that format", function() {
    expect(dateParser.parseAsMySqlDate("2013-06-07")).to.equal("2013-06-07");
  });
});
